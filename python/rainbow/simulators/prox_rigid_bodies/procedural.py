...
def create_rockslide(
    ...
    V, T = igl.read_triangle_mesh("../data/landscape.obj")

    mesh = API.create_mesh(V, T)
    API.create_shape(engine, shape_name, mesh)

    body_name = API.generate_unique_name("ground")
    body_names.append(body_name)
    API.create_rigid_body(engine, body_name)
    API.connect_shape(engine, body_name, shape_name)

    API.set_position(engine, body_name, V3.zero(), False)
    API.set_orientation(engine, body_name, Q.identity(), False)
