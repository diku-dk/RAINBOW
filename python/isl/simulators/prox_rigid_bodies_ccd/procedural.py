...
def create_tower(engine,
    ...
    if use_cubes:
        ...
        brick_inner_half_width = (sin(delta_phi / 2.0) / sin(pi / 2.0 - delta_phi / 2.0)) * inner_radius * safety_factor
        brick_outer_half_width = (sin(delta_phi / 2.0) / sin(pi / 2.0 - delta_phi / 2.0)) * outer_radius * safety_factor
...
def create_funnel(engine,
    ...
    q = Q.identity()
    body_names = _create_grid(engine,
                              r,
                              q,
                              shape_names,
                              width,
...
def create_poles(engine,
    ...
    shape_names = []
    obj_files = ['../data/cow.obj', '../data//dragon.obj', '../data//knot.obj',
                 '../data//lamp.obj', '../data//propella.obj', '../data//shuttle.obj']
...
def create_temple(engine,
    ...
    V, T = MESH.create_box(temple_width + 6 * pillar_width, stairs_height, temple_depth + 6 * pillar_depth)
    mesh = API.create_mesh(V, T)
    API.create_shape(engine, shape_name, mesh)
    ...
    V, T = MESH.create_box(temple_width + 4 * pillar_width, stairs_height, temple_depth + 4 * pillar_depth)
    mesh = API.create_mesh(V, T)
    API.create_shape(engine, shape_name, mesh)
    ...
    V, T = MESH.create_box(temple_width + 2 * pillar_width, stairs_height, temple_depth + 2 * pillar_depth)
    mesh = API.create_mesh(V, T)
    API.create_shape(engine, shape_name, mesh)
    ...
    V, T = MESH.create_box(temple_width - 2.5 * pillar_width, stairs_height, temple_depth - 2.5 * pillar_depth)
    mesh = API.create_mesh(V, T)
    API.create_shape(engine, shape_name, mesh)
    ...
    for k in range(3, K_pillars - 3, 3):
        ...
        for i in range(1, I_pillars - 1):
            ...
            r_g = V3.make(x, y, z)
            q_g = Q.identity()
            names = create_pillar(engine, r_g, q_g, pillar_width, pillar_height - stairs_height, pillar_depth,
                                  pillar_stones, density,
                                  material_name)
            body_names.extend(names)
    ...
    for k in range(0, K_pillars, 3):
        ...
        for j in range(I_pillars - 1):
            for i in range(I_pillars - j):
                ...
                x_brick = -temple_width / 2.0 + pillar_width / 2.0 +\
                    i * gable_brick_width + j * gable_brick_width * 0.5
                y_brick = 3.0 * stairs_height + pillar_height +\
                    epistyle_height + gable_brick_height / 2.0 + j * gable_brick_height
...
def create_sandbox(engine,
    ...
    for n in range(20):
        ...
        mesh = API.create_mesh(V, T)
        MESH.scale_to_unit(mesh)
        s = min(box_width / I_grains, box_height / J_grains,
                box_depth / K_grains) * 0.9  # The 0.9 scaling ensure some padding to avoid initial contact
        MESH.scale(mesh, s, s, s)
        API.create_shape(engine, shape_name, mesh)
    ...
    V, T = MESH.create_box(thickness * 2.0 + box_width, thickness, box_depth + 2.0 * thickness)
    mesh = API.create_mesh(V, T)
    API.create_shape(engine, shape_name, mesh)
