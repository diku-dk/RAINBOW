...
def prox_ellipsoid(engine, z_s, z_t, z_tau, mu_s, mu_t, mu_tau, x_n):
    ...
    sa, sb, sc, sx, sy, sz = scale * a, scale * b, scale * c, scale * z_s, scale * z_t, scale * z_tau
    ...
    g0 = (saa * sxx)/((saa + t0)**2) + (sbb * syy)/((sbb + t0)**2) + (scc * szz)/((scc + t0)**2) - 1
    g1 = (saa * sxx)/((saa + t1)**2) + (sbb * syy)/((sbb + t1)**2) + (scc * szz)/((scc + t1)**2) - 1
    while g1 > 0:
    ...
    for iteration in range(engine.params.ellipsoid_max_iterations):
        ...
        if abs(t1 - t0) < engine.params.ellipsoid_tolerance:
            break
        gk = (saa * sxx)/((saa + tk)**2) + (sbb * syy)/((sbb + tk)**2) + (scc * szz)/((scc + tk)**2) - 1
...
def sweep(K, J, WJT, b, mu, r, x, friction_solver, engine):
    ...
    for k in range(K):
        ...
        x_b = x[block]
        delta = x_b.copy()  # Used to keep the old values and compute the change in values
        r_b = r[block]
        b_b = b[block]
        # By definition
        #       z = x - r (J WJ^T x  + b)
        ...
        x_b[1], x_b[2], x_b[3] = friction_solver(engine, z_b[1], z_b[2], z_b[3], mu_k, mu_k, mu_k, x_b[0])
...
def solve(J, WJT, b, mu, friction_solver, engine, stats, debug_on, prefix):
    ...
    if debug_on:
        ...
        stats[prefix + 'residuals'] = np.ones(engine.params.max_iterations, dtype=np.float64)*np.inf
        stats[prefix + 'lambda'] = np.zeros([engine.params.max_iterations]+list(b.shape), dtype=np.float64)
    ...
    sol = np.zeros(b.shape, dtype=np.float64)  # The last best known solution, used for restarting if divergence
